...
def compute_mean_std(
    ...
    #print(mean.shape, feats.shape, std.shape)
    return (x - mean) / std
class Model(nn.Module):
    ...
    def __init__(self, lda_out_channels, hyper_in_channels, target_in_size, target_fc1_size, target_fc2_size, target_fc3_size, target_fc4_size, feature_size):
        ...
        # self.beta = nn.Linear(1, 2048)
        # initialize
        for i, m_name in enumerate(self._modules):
            if i > 2:
                nn.init.kaiming_normal_(self._modules[m_name].weight.data)

        self.gamma = nn.Linear(1, self.hyperInChn)
        self.beta = nn.Linear(1, self.hyperInChn)

        # self.conv2 = nn.Sequential(
        #     nn.Conv2d(self.target_in_size*2, self.target_in_size, 1, padding=(0, 0)),
        #     nn.ReLU(inplace=True),
        # )
        self.gamma1 = nn.Sequential(nn.Linear(1, self.hyperInChn),nn.ReLU())
    def forward(self, img):
        ...
        target_in_vec = res_out['target_in_vec'].view(-1, self.target_in_size, 1, 1) #+ gamma.unsqueeze(-1).unsqueeze(-1)

        #gamma = gamma.unsqueeze(-1).unsqueeze(-1).repeat(img.shape[0]//gamma.shape[0],1,1,1)

        #target_in_vec = self.conv2(torch.cat((target_in_vec, gamma),dim=1))
        #print(res_out['hyper_in_feat'].shape)
        #res_out['hyper_in_feat'] = 
        # input features for hyper net
        hyper_in_feat = self.conv1(res_out['hyper_in_feat']).view(-1, self.hyperInChn, feature_size, feature_size) #+ self.gamma1(z).unsqueeze(-1).unsqueeze(-1)

        hyper_in_feat = compute_mean_std(hyper_in_feat) *  gamma.unsqueeze(-1).unsqueeze(-1) + beta.unsqueeze(-1).unsqueeze(-1)   
 #       hyper_in_feat = hyper_in_feat 
...
class ResNetBackbone(nn.Module):
    ...
    def __init__(self, lda_out_channels, in_chn, block, layers, num_classes=1000):
        ...
        self.conv_ = nn.Conv2d(2048, in_chn, kernel_size=1, stride=1, padding=0, bias=False)
