...
class ResidualBlock_CRes(nn.Module):
    ...
    def forward(self, x):
        ...
        return identity + out * local_scale.unsqueeze(-1).unsqueeze(-1) 
...
class Model(nn.Module):
    ...
    def __init__(self, lda_out_channels, hyper_in_channels, target_in_size, target_fc1_size, target_fc2_size, target_fc3_size, target_fc4_size, feature_size):
        ...
        #     nn.Conv2d(self.hyperInChn*2, self.hyperInChn, 1, padding=(0, 0)),
        #     nn.ReLU(inplace=True)
        #     )
    ...
    def forward(self, img):
        img, z = img 
        z = self.beta(z).unsqueeze(-1).unsqueeze(-1)
        z = z.repeat(img.shape[0]//z.shape[0],1,7,7)
        ...
        res_out['hyper_in_feat'] = z #torch.cat((res_out['hyper_in_feat'], z), dim=1) 
        ...
        # hyper_in_feat = self.conv2(torch.cat((hyper_in_feat,z), dim=1))
