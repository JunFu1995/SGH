...
class CVIU(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        imgP = imgPath.split('/')
        imgPtx = 'text_'+imgP[-1]
        textPath = '/'.join(imgP[:-1] + [imgPtx])
        imgPsx = 'struct_'+imgP[-1]
        structPath = '/'.join(imgP[:-1] + [imgPsx])
        #refsPath = imgPath[:-len(imgPath.split('/')[-1])] + 'O.bmp'
        text = pil_loader(textPath)
        struct = pil_loader(structPath)

        if self.training: 
            i, j, h, w = torchvision.transforms.RandomCrop.get_params(
            text, output_size=(224, 224))
            text = TF.crop(text, i, j, h, w)
            struct  = TF.crop(struct, i, j, h, w)

        if self.transform is not None:
            text = self.transform(text)
            struct = self.transform(struct)

        sample = torch.cat((text, struct), dim=0)
        return imgPath, sample, mos, z #sample, ref, mos
    def __len__(self):
